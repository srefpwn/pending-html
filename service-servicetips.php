    ...
    <?php foreach ($serviceTipsData['topics'] as $topic): ?>
        ...
        ?>
        												<table align="center" class="table-border" style="width:100%;">
                   		 									<tr>
                        										<td style="background-color:#bb271a;color:#ffffff;width:100%;text-align:center;" class="p10">
...
<?php if (($item['type'] ?? 'text') === 'part_number'): ?>
    <?php if (
        ...
        isset($epcPartNumbers[$value])
       	 														...
       	 														<a href="/epc/search.php?type=<?= urlencode($epcPage) ?>&part_number=<?= urlencode($value) ?><?= $navigationParams ?>"><?= htmlspecialchars($value) ?>
        														</a>
    <?php else: ?>
