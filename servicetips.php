...
if (
    ...
    if ($epcEnabled) {
        ...
        $brand = trim((string)($car['brand'] ?? ''));
        $model = trim((string)($car['model'] ?? ''));
        $series = trim((string)($vinConfig['series'] ?? ''));
        $modelCode = trim((string)($vinConfig['model_code'] ?? ''));
                                        ...
                                        <tr>
                                            <td class="textv-top">
                                                <table align="center" class="table-border" width="100%">
                                                    ...
                                                    <tr>
                                                        <td style="padding:20px;text-align:center; min-height:250px">
    ...
    <?php foreach ($serviceTipsData['topics'] as $topic): ?>
        ...
        ?>
        												<table align="center" class="table-border" style="width:100%;">
                   		 									<tr>
                        										<td class="pl10 epc-text row-odd p5 text-left" style="background-color:#bb271a;color:#ffffff;width:100%;text-align:center;">
                    ...
                    <?php foreach ($items as $index => $item): ?>
                        ...
                        ?>
                       	 									<tr>
                            									<td class="pr5 epc-text <?= $rowClass ?> p5 pl10">
                            									<?= htmlspecialchars($label) ?>:
                            									</td>
                            									<td class="epc-text <?= $rowClass ?> p5 text-right pr10">      
<?php if (($item['type'] ?? 'text') === 'part_number'): ?>
    <?php if (
        $epcEnabled &&
        isset($epcPartNumbers[$value])
    ): ?>
       	 														<a href="/epc/search.php?brand=<?= urlencode($epcConfig['brand']) ?>&model=<?= urlencode($epcConfig['model']) ?>&series=<?= urlencode($epcConfig['series']) ?>&model_code=<?= urlencode($epcConfig['model_code']) ?>&body_code=<?= urlencode($epcConfig['body_code']) ?>&trim_code=<?= urlencode($epcConfig['trim_code']) ?>&car=<?= (int)$carId ?>&part_number=<?= urlencode($value) ?><?= $navigationParams ?>"><?= htmlspecialchars($value) ?>
        														</a>
    <?php else: ?>
