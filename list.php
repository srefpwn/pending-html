...
$bodyCode = strtolower(
    trim((string)($_GET['body_code'] ?? ''))
);

$trimCode = strtolower(
    trim((string)($_GET['trim_code'] ?? ''))
);
                				...
                				<!-- Keresés -->
               						<tr>
                            			...
                            			<table align="center">
                                			<tr>
												...
												<input type="hidden" name="body_code" value="<?= htmlspecialchars($bodyCode) ?>">
												<input type="hidden" name="trim_code" value="<?= htmlspecialchars($trimCode) ?>">
									...
									<tr>
										...
										<table id="manual-book" align="center">
											<tr>
   			 									<td>
        										<a href="?brand=<?= urlencode($brand) ?>&model=<?= urlencode($model) ?>&series=<?= urlencode($series) ?>&model_code=<?= urlencode($modelCode) ?>&body_code=<?= urlencode($bodyCode) ?>&trim_code=<?= urlencode($trimCode) ?>&year=<?= (int)$year ?><?= $navigationParams ?>&book=manual" data-book="manual" class="<?= $book !== 'manual' ? 'greybutton' : '' ?>">
        										<button type="submit">Javítási könyvek</button>
        										</a>
    											</td>
												<td>
        										<a href="?brand=<?= urlencode($brand) ?>&model=<?= urlencode($model) ?>&series=<?= urlencode($series) ?>&model_code=<?= urlencode($modelCode) ?>&body_code=<?= urlencode($bodyCode) ?>&trim_code=<?= urlencode($trimCode) ?>&year=<?= (int)$year ?><?= $navigationParams ?>&book=body" data-book="body" class="<?= $book !== 'body' ? 'greybutton' : '' ?>">
                                        ...
                                        <table class="menutable" width="100%">
                        					<tr>
                        						...
                        						<table id="manual-category" class="table-border">
                                    				...
                                    				<tr>
                                    					<td>
                                        				<a href="?brand=<?= urlencode($brand) ?>&model=<?= urlencode($model) ?>&series=<?= urlencode($series) ?>&model_code=<?= urlencode($modelCode) ?>&body_code=<?= urlencode($bodyCode) ?>&trim_code=<?= urlencode($trimCode) ?>&year=<?= (int)$year ?><?= $navigationParams ?>&category=<?= $categoryId ?>&book=<?= urlencode($book) ?>" data-category="<?= $categoryId ?>" class="<?= $isActive ? '' : 'greybutton' ?>">
