    ...
    if ($epcEnabled) {
        ...
        foreach ($configs as $item) {
            ...
            if (
                ...
                ($item['series'] ?? '') === ($car['series'] ?? '') &&
                ($item['model_code'] ?? '') === ($vinConfig['model_code'] ?? '') &&
                ($item['body_code'] ?? '') === ($vinConfig['body_code'] ?? '') &&
                ($item['trim_code'] ?? '') === ($vinConfig['trim_code'] ?? '')
            ) {
                $epcConfig = $item;
                break;
            }
                            ...
                            <tr>
                                <td>
                                    <table align="left" width="100%" class="table-border">
                                        <tr>
                                            <td class="textv-top">
                                                <table id="print-area" align="center" class="table-border" width="100%">
                                                    <!-- Cím -->
													<tr>
													...
													<tr>
														<td id="print-button" style="padding:20px; text-align:center;">
													...
													<tr>
														...
														<!-- Alkatrészek -->
														<table align="center" style="width:100%;text-align:center;" class="table-border">											
															<tr>
																<td class="pl10 epc-text row-odd p5 text-left" width="15%" style="background-color:#bb271a;color:#ffffff;">Cikkszám
																</td>
																...
																<td class="pr5 epc-text row-odd p5" width="15%" style="background-color:#bb271a;color:#ffffff;">Darab
																</td>
																<td class="pr10 epc-text row-odd p5 text-right" width="8%" style="background-color:#bb271a;color:#ffffff;">Összesen
																</td>
															</tr>
...
?>
															<tr>
																...
																<?php if (isset($epcPartNumbers[$partNumber])): ?><a href="/epc/search.php?brand=<?= urlencode($epcConfig['brand']) ?>&model=<?= urlencode($epcConfig['model']) ?>&series=<?= urlencode($epcConfig['series']) ?>&model_code=<?= urlencode($epcConfig['model_code']) ?>&body_code=<?= urlencode($epcConfig['body_code']) ?>&trim_code=<?= urlencode($epcConfig['trim_code']) ?>&car=<?= (int)$carId ?>&part_number=<?= urlencode($partNumber) ?><?= $navigationParams ?>"><?= htmlspecialchars($partNumber) ?></a><?php else: ?><?= htmlspecialchars($partNumber) ?><?php endif; ?></td>
