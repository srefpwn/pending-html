			...
			<table align="left" width="100%" class="table-border">
				<tr>
					...
					<?php if ($userCar !== null): ?>
						<tr>
							...
							<?php if (trim((string)($userCar['name'] ?? '')) !== ''): ?>
           					<span class="epc-title"><?= htmlspecialchars($userCar['name']) ?> - </span>
        					<?php endif; ?>
        					<span class="epc-title"><?= htmlspecialchars($userCar['vin']) ?> - </span>
        					<span class="epc-title"><?php if (trim((string)($selectedCategory ?? '')) !== '' && $selectedCategory !== 'all'): ?><?= htmlspecialchars($selectedCategory) ?> - <?php endif; ?></span>
					...
					<?php else: ?>
						<tr>
    						<td style="padding:0px;text-align:center;">
        					<span class="epc-title"><?= htmlspecialchars($config['name']) ?> - </span>
        					<span class="epc-title"><?php if (trim((string)($selectedCategory ?? '')) !== '' && $selectedCategory !== 'all'): ?><?= htmlspecialchars($selectedCategory) ?> - <?php endif; ?></span>
        					<span class="epc-title">EPC</span>
        					</td>
						</tr>
					<?php endif; ?>
						...
						<tr>
							...
							<table align="center">
								<tr>
									<td><a href="?page=<?= urlencode($page) ?><?= $navigationParams ?>&category=all"><button type="submit">Összes</button></a></td>
           							<?php foreach ($categories as $category): ?>
                					<td>
                    				<a href="?page=<?= urlencode($page) ?><?= $navigationParams ?>&category=<?= urlencode($category) ?>"><button type="submit"><?= htmlspecialchars($category) ?></button></a>
                					</td>
            						<?php endforeach; ?>
								</tr>
							</table>
