...
$jsDir = __DIR__ . '/js';
$htmlTargetDir = $_SERVER['DOCUMENT_ROOT'] . '/data/manual/honda/accord/series_7/html';
$jsonTargetDir = $_SERVER['DOCUMENT_ROOT'] . '/data/manual/honda/accord/series_7/json';
...
function convertImagePaths(string $html): string
{
    return preg_replace_callback(
        ...
        function ($match) {
            ...
            return $match[1]
                . '/images/manual/honda/accord/series_7/'
...
function fixHondaVmlContainerSizes(string $html): string
{
     ...
     *
     * Jelenleg háromféle Honda struktúrát kezelünk:
     *
     * 1. ViewerTD
     ...
     *    </div>
     *
     * 3. Egyszerű DIV + imgDispId + IMG
     *    <div>
     *        <span style="display:block" id="imgDispId">
     *            <a ...>
     *                <img src="...">
     *            </a>
     *        </span>
     *    </div>
     *
     * A harmadik esetben csak akkor módosítjuk az IMG-t,
     * ha semmilyen width / height méret nincs megadva.
     */
     ...
     * ============================================================
     *
     * A stabilan működő TD + VML logika.
     */
    ...
    if (!empty($tdMatches[0])) {
        ...
        foreach ($tdMatches[0] as $tdMatch) {
            ...
            /*
             * Nincs szükség rtrim()-re.
             *
             * A Honda VML méretei egész pixelértékek.
             * A korábbi rtrim(..., '0') hibásan például:
             *
             * 340 -> 34
             * 320 -> 32
             *
    ...
    if (!empty($divMatches[0])) {
        ...
        foreach ($divMatches[0] as $divMatch) {
            ...
            /*
             * A számokat nem formázzuk rtrim()-mel.
             */
            $widthValue  = (string)$width;
            $heightValue = (string)$height;
    ...
    /*
     * ============================================================
     * 3. Méret nélküli imgDispId kép -> width="475"
     * ============================================================
     *
     * Csak az alábbi, nagyon szigorúan meghatározott struktúrát
     * kezeljük:
     *
     * <div>
     *     <span style="display:block" id="imgDispId">
     ...
     * Feltételek:
     *
     * - a DIV teljesen attribútum nélküli;
     * - a DIV-ben nincs script;
     * - van imgDispId azonosítójú SPAN;
     * - a SPAN-on belül van A;
     * - az A-n belül van IMG;
     * - az IMG-nek nincs width attribútuma;
     * - az IMG-nek nincs height attribútuma;
     * - az IMG style attribútumában nincs width;
     * - az IMG style attribútumában nincs height.
     *
     * Csak ebben az esetben adunk:
     *
     * width="475"
     *
     * A DIV-et, SPAN-t és A taget nem módosítjuk.
     */

   preg_match_all(
    '#<div>\s*'
    . '<span\b[^>]*\bid\s*=\s*(["\'])imgDispId\1[^>]*>\s*'
    . '<a\b[^>]*>\s*'
    . '<img\b[^>]*>\s*'
    . '</a>\s*'
    . '</span>\s*'
...
if (!empty($imageDivMatches[0])) {

    foreach ($imageDivMatches[0] as $imageDivMatch) {

        $imageDivBlock = $imageDivMatch[0];
        $imageDivStart = $imageDivMatch[1];

        /*
         * Biztonsági feltétel:
         * ebben a DIV-ben semmilyen script nem lehet.
         */
        if (preg_match(
            '#<script\b#i',
            $imageDivBlock
        ...
        /*
         * Keressük meg az IMG nyitó tagjét.
         */
        if (!preg_match(
            '#<img\b[^>]*>#i',
            $imageDivBlock,
        ...
        $imgOpenTag     = $imgMatch[0][0];
        $imgRelativePos = $imgMatch[0][1];

        /*
         * Ha az IMG-nek width vagy height attribútuma van,
         * nem módosítjuk.
         */
        if (preg_match(
            '#\b(?:width|height)\s*=#i',
        ...
        /*
         * Ha az IMG style attribútumában width vagy height
         * szerepel, szintén nem módosítjuk.
         */
        if (preg_match(
            '#\bstyle\s*=\s*(["\'])[^"\']*\b(?:width|height)\s*:#i',
            $imgOpenTag
        )) {
            continue;
        }
         ...
         *
         * Csak az IMG taget módosítjuk.
         */
        $newImgOpenTag =
            substr($imgOpenTag, 0, -1)
            . ' width="475">';

        $replacements[] = [
            'start'       => $imageDivStart + $imgRelativePos,
            'length'      => strlen($imgOpenTag),
            'replacement' => $newImgOpenTag
        ];
    }
}
    ...
    /*
     * ============================================================
     * 4. Az összes módosítás végrehajtása
     * ============================================================
     *
...
JS;
    ...
    return '<div class="manual-content-iframe" style="width:100%;margin:0;padding:0;overflow:hidden;">'
        . '<iframe id="honda-iframe" src="/data/manual/honda/accord/series_7/html/'
...
function createWiringDiagramIframe(string $id): string
{
    ...
    return '<div class="manual-wiring-diagram" style="width:100%;margin:0;padding:0;overflow:hidden;">'
        . '<iframe src="/data/manual/honda/accord/series_7/html/'
        . rawurlencode($contentFile)
