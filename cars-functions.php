...
if (isset($rule['engine_values']) && is_array($rule['engine_values'])) {

    $engine = $vinValues['engine'] ?? null;

    if ($engine !== null) {
        $engineValues = $rule['engine_values'][$engine] ?? [];

        if (is_array($engineValues)) {
            $result = $engineValues[$value] ?? null;
        }
    }
